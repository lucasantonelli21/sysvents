            ...
            <div class="left-section">
                ...
                <form method="GET" action="{{ url('/eventos') }}">
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <input type="text" class="form-control filter" placeholder="Nome do evento" name="name">
                    </div>

                    <div class="mb-3 form-group">
                        <label class="mb-0 form-label">Pesquisar pela categoria</label>
                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['technology'] }}">
                            <label for="{{ $themes['technology'] }}" class="form-check-label">{{ $themes['technology'] }}</label>
                        </div>

                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['cultural'] }}">
                            <label for="{{ $themes['cultural'] }}" class="form-check-label">{{ $themes['cultural'] }}</label>
                        </div>

                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['musical'] }}">
                            <label for="{{ $themes['musical'] }}" class="form-check-label">{{ $themes['musical'] }}</label>
                        </div>

                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['art'] }}">
                            <label for="{{ $themes['art'] }}" class="form-check-label">{{ $themes['art'] }}</label>
                        </div>

                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['sport'] }}">
                            <label for="{{ $themes['sport'] }}" class="form-check-label">{{ $themes['sport'] }}</label>
                        </div>

                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['gastronomy'] }}">
                            <label for="{{ $themes['gastronomy'] }}" class="form-check-label">{{ $themes['gastronomy'] }}</label>
                        </div>

                        <div class="form-check">
                            <input name="theme" type="radio" class="form-check-input" id="{{ $themes['health'] }}">
            ...
            <div class="right-section ms-3 p-3">
                ...
                <div class="d-flex justify-content-between flex-wrap gap-3">
                    @foreach ($events as $event)
                        <a id="{{ 'evento'.$event->id }}" class="evento media-element mb-1 rounded" href="{{ url('eventos/'.$event->id) }}">
